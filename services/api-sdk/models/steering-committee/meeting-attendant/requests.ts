 ...
 * @param meetingId - The id of the session administrator.
...
export const getAllMeetingAttendants = async (meetingId) => {
  const response = await fetchService.get(
    `/list_attendance_session_administrators?session_administrator_id=${meetingId}`,
  );
  if (response.ok) {
...
export const updateMeetingAttendantsStatus = async (meetingId, args) => {
  const response = await fetchService.post(
    "/update_attendance_session_administrators",
    JSON.stringify({
      session_administrator_id: meetingId,
      administrators: args.attendants.map(({ id, type, isAttending }) => ({
        id,
        type,
        status: isAttending,
      })),
...
/**
 * Prints the list of attendants of a session administrator.
 * @param meetingId - The id of the session administrator.
 * @throws {ApiError} if the request fails.
 */
export const printMeetingAttendantsList = async (meetingId) => {
  const response = await fetchService.get(
    `/generate_pdf_attendance_session_administrators?session_administrator_id=${meetingId}`,
  );
  if (!response.ok) {
