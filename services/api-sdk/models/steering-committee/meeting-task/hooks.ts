...
import { useOneSteeringCommitteeInvalidate } from "@/services/api-sdk/models/steering-committee/steering-committee";
import {
  createMeetingTask,
  deleteMeetingTask,
  forwardMeetingTask,
  ...
  const getMutationOptions = useCallback(
    (mutateOptions) => ({
      ...
      onSettled: async (...args) => {
        ...
        await invalidateAllMeetingTasks();
        if (args[0]?.meetingId) {
          await invalidateOneSteeringCommittee(args[0].meetingId);
        }
        return mutateOptions?.onSettled?.(...args);
      },
    }),
    [
      invalidateAllMeetingTasks,
      invalidateOneSteeringCommittee,
