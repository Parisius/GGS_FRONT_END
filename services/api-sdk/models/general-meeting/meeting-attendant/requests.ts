...
export const getAllMeetingAttendants = async (meetingId) => {
  const response = await fetchService.get(
    `/list_attendance_general_meetings?general_meeting_id=${meetingId}`,
  );
  if (response.ok) {
...
export const updateMeetingAttendantsStatus = async (meetingId, args) => {
  const response = await fetchService.post(
    "/update_attendance_general_meetings",
    JSON.stringify({
      general_meeting_id: meetingId,
      shareholders: args.attendants.map(({ id, type, isAttending }) => ({
        id,
        type,
        status: isAttending,
      })),
...
/**
 * Prints the list of attendants of a general meeting.
 * @param meetingId - The id of the general meeting.
 * @throws {ApiError} if the request fails.
 */
export const printMeetingAttendantsList = async (meetingId) => {
  const response = await fetchService.get(
    `/generate_pdf_attendance_general_meetings?general_meeting_id=${meetingId}`,
  );
  if (!response.ok) {
